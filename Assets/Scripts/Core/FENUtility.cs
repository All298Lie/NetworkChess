...
namespace NetworkChess.Core
{
    ...
    {
        public static string GeneratingStateString(CorePiece[,] board, bool isWhiteTurn, BoardPos? enPassantPos)
        {
            ...
            // 4. 앙파상 가능한 칸
            if (enPassantPos.HasValue == true)
            {
                BoardPos ep = enPassantPos.Value;
        ...
        private static char GetPieceChar(CorePiece piece)
        {
            char c;
        ...
        private static string GetCastlingRight(CorePiece[,] board, bool isWhite, bool isKingSide)
        {
            BoardPos kingPos = MoveValidator.FindKingPosition(board, isWhite);

            // 1. 킹이 존재하지 않거나 킹이 움직였을 경우
            if (kingPos.x == -1) return "";

            // 2. 룩 위치 확인
            CorePiece targetRook = null;
            if (isKingSide == true)
            {
                for (int x = kingPos.x + 1; x < 8; x++)
                {
                    CorePiece p = board[x, kingPos.y];
